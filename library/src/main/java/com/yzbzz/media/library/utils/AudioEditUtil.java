...
public class AudioEditUtil {
    ...
    public static String cutAudio(String path, Audio audio, float cutStartTime, float cutEndTime) {
        ...
        //重命名为源文件
        FileUtils.renameFile(new File(tempOutPath), tempOutPathWAV);

        return tempOutPathWAV;
    }
     ...
     * @param cutEndTime   裁剪结束时间
     */
    public static String cutAudioNoDelete(Audio audio, float cutStartTime, float cutEndTime) {
        if (cutStartTime == 0 && cutEndTime == audio.getTimeMillis() / 1000f) {
            return "";
        }
        if (cutStartTime >= cutEndTime) {
        ...
        String tempOutPath = srcWavePath + cutEndTime + ".temp";
        File file1 = new File(srcWavePath);
        String tempOutPathWAV = file1.getParent() + "/" + cutEndTime + ".wav";
        try {
        ...
        String path = tempOutPathWAV;

        File file = new File(tempOutPathWAV);
        String lj = file.getParent() + "/original/";

        String name = path.substring(path.lastIndexOf("/") + 1, path.lastIndexOf("."));

        String pcmFilePath = lj + name + ".pcm";
        String aacFilePath = lj + name + ".aac";


        AudioEncodeUtil.convertWav2Pcm(tempOutPathWAV, pcmFilePath);
